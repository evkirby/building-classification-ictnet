...
ROTATION_FLIP_AUGMENTATION = True
CONTRAST_BRIGHTNESS_AUGMENTATION = True
SCALE_AUGMENTATION = True

smallest_patch_size = (PATCH_SIZE // 4) * 3
largest_patch_size = (PATCH_SIZE // 2) * 3 + 1
...
def get_training_patches(images, ground_truths, x_list, y_list, mean_r, mean_g, mean_b):
    ...
    for img_num in range(len(images)):
        ...
        if DATA_AUGMENTATION and ROTATION_FLIP_AUGMENTATION:
        ...
        if DATA_AUGMENTATION and CONTRAST_BRIGHTNESS_AUGMENTATION:
        ...
        for patch_num in range(len(x_list)):
            x = x_list[patch_num]
            y = y_list[patch_num]
            if not DATA_AUGMENTATION or (DATA_AUGMENTATION and not SCALE_AUGMENTATION):
...
def resize_image(img, gt):
    new_img = cv2.resize(img, (PATCH_SIZE, PATCH_SIZE), interpolation = cv2.INTER_CUBIC)
    new_gt = cv2.resize(gt, (PATCH_SIZE, PATCH_SIZE), interpolation = cv2.INTER_CUBIC)
