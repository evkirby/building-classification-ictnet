...
def dense_block(inputs, num_layers, n_filters, growth_rate, dropout_rate, path_type):
    new_features = []
    for i in range(num_layers): # num_layers is the value of 'l'
        conv_outputs = pre_activation_conv(inputs, n_filters, dropout_rate=dropout_rate)
...
def build_model(preset_model='FC-DenseNet56', num_classes=2, n_filters_first_conv=48, n_pool=5, growth_rate=12, n_layers_per_block=4, dropout_rate=0.2):
    ...
    for i in range(n_pool):
        ...
        # Dense Block
        x, _ = dense_block(x, n_layers_per_block[i], n_filters, growth_rate, dropout_rate, "down")
    ...
    x, block_to_upsample = dense_block(x, n_layers_per_block[n_pool], out_dim, growth_rate, dropout_rate, "bottlneck")
    ...
    for i in range(n_pool):
        ...
        # We will only upsample the new feature maps
        x, block_to_upsample = dense_block(x, n_layers_per_block[i], out_dim, growth_rate, dropout_rate, "up")
